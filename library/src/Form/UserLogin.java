...
public class UserLogin extends JFrame implements Runnable {
    ...
    public void run() {
        ...
        login.addActionListener(new UserLoginAction());				//添加监听器
    ...
    private class UserLoginAction implements ActionListener {
        public void actionPerformed(final ActionEvent e) {
            ...
            user = query.UserLoginCheck(username.getText(),String.valueOf(password.getPassword()));
            if (user.getName() != null) {//判断用户名是否为null
                try {
                    //Main frame = new Main();//创建一个主窗体
                    UserMode frame = new UserMode();
                    frame.setVisible(true);//设置其可见
                    UserLogin.this.setVisible(false);//设置登录窗体为不显示
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            } else {
